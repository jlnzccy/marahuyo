...
- [x] Supabase schema sync via GitHub Action — `.github/workflows/supabase-db-push.yml` runs `supabase db push` on every push to main that touches `supabase/migrations/**`. Requires `SUPABASE_ACCESS_TOKEN`, `SUPABASE_PROJECT_REF`, `SUPABASE_DB_PASSWORD` as repo secrets.
- [x] Brute-force protection on `/studio/login` — already complete via Phase 5 🔴 (5/15min in-memory lockout in `src/app/studio/actions.ts`).
...
- [x] **Skeleton/blur for images.** New `src/components/blur-image.tsx` fades the bitmap in once decoded (inline `opacity` style so it composes with hover-transform). Wired into `WorkRow` + `FeaturedCard`. Container `bg-surface` skeleton shows beneath while loading.
...
- [x] **Slug i18n.** Documented the ASCII-only behaviour in `src/lib/slug.ts` JSDoc — CJK/emoji are stripped intentionally; multilingual support would swap the filter for `\p{Letter}\p{Number}` with the `u` flag and percent-encode at the URL layer.
...
- [x] **Poetry-block keyboard exit.** `poetry-node.ts` now (a) exits to a fresh paragraph when Enter is pressed at the end of the node and the previous character is already `\n`, and (b) binds `Mod-Enter` as an unambiguous escape regardless of cursor position.
...
- [x] **`tsconfig.skipLibCheck`.** Tried removing — surfaces JSX namespace errors from `@dnd-kit/*`, `Database` generic-constraint errors from `@supabase/ssr`, and a missing `ReactSVG` export from `lucide-react`. All require upstream/peer-dep fixes; reverted with `skipLibCheck: true` retained.
...
- [x] **Reader-actions placement.** New `src/components/reader-actions.tsx` mounts outside the prose: fixed left-side vertical pill on `lg+`, fixed bottom-center horizontal pill on mobile. Backdrop-blurred surface, `no-print` so paper output stays clean. Removed from inline FadeUp slot in `reader-shell.tsx`.

### 🔵 Feature ideas (defer, scope per session)

- [x] **Full-text search.** Migration `0014_search.sql` adds generated `search_tsv` columns on works + chapters (title A, subtitle/excerpt B, body C) plus GIN indexes. `src/lib/search.ts` translates user input to `to_tsquery` with prefix matching on the trailing token. `/search` route renders ranked hits with client-side highlighted snippets. "Search" added to primary nav.
- [x] **Table of contents.** `src/lib/toc.ts` stamps stable ids on h2/h3 headings of the sanitized body. `src/components/table-of-contents.tsx` renders a `<details>` collapsible on mobile and a fixed sticky sidebar on `lg+`, with IntersectionObserver-driven scroll-spy. Skipped when fewer than 2 headings. `scroll-margin-top: 6rem` keeps anchors clear of the sticky header.
- [x] **Reading position resume.** `Bookmark.scrollPercent` added; `BookmarkTracker` debounces scroll listener + `visibilitychange` flush; `ContinueReading` paints a tiny progress bar + "Continue from N%" (or "Almost finished" past 95%).
- [x] **Version history.** Migration `0015_work_versions.sql` creates `work_versions(work_id, title, subtitle, excerpt, body, saved_at)`. `captureWorkVersion()` snapshot fires from `publishWork` AND `restoreWorkVersion` (so rollbacks are reversible). `/studio/works/[id]/versions` lists snapshots with relative + absolute timestamps and a `Restore` button. Editor header links to history.
- [x] **Tag filters.** `/works` reads `?tag=` searchParam, builds a deduped tag list with counts, renders filter chips above the archive list (active chip = inverted), filters works by tag (case-insensitive). Empty-state copy adapts when no work matches the selected tag.
- [x] **Auto theme.** New `ThemePreference` type (`auto | light | cream | midnight`) layered above the concrete `Theme`. ThemeProvider listens to `matchMedia('(prefers-color-scheme: dark)')` while preference is auto. Pre-hydration script resolves auto → midnight/light before paint. ThemeSwitcher trigger shows `Monitor` icon while auto is selected; radiogroup gains a fourth "Auto" entry. Auto is the new default.
- [x] **Explicit `featured` flag.** Migration `0012_works_featured.sql` adds `featured boolean default false` + partial index. `getFeaturedWork()` picks the newest published `featured=true` row, falling back to overall newest (tolerates missing column for the deploy/migration window). Sidebar toggle wired into the work editor.
- [x] **Scheduled publishing.** Migration `0016_scheduled_publish.sql` adds `scheduled_at timestamptz` + partial index. `updateWorkSchema` accepts ISO datetimes. `/api/cron/publish` (Bearer-auth via `CRON_SECRET`) sweeps drafts whose schedule has elapsed and flips them to published, revalidating reading surfaces. `vercel.json` runs the cron every 15 min. Editor sidebar exposes a `datetime-local` picker.
- [x] **Bookmarks/likes sync.** Migration `0017_reader_state.sql` adds an anonymous `reader_state(device_id, key, …)` table (service-role only — RLS denies anon). `src/lib/device-id.ts` mints a per-browser UUID into localStorage. `src/lib/reader-sync.ts` posts via `navigator.sendBeacon` (with fetch+keepalive fallback). `BookmarkTracker` + `LikeButton` fire-and-forget upserts. `ReaderStateHydrator` (mounted in root layout) pulls server state once every 24h and merges into local — local writes win on conflict.
- [x] **Bulk actions in studio.** `bulkUpdateWorks` + `bulkUpdateChapters` server actions accept ids + action (`publish`/`unpublish`/`delete`). `BulkWorksList` renders `/studio/works` with checkboxes + sticky bottom action bar; `BulkDraftsList` does the same for `/studio/drafts`, routing each id to the right table.
- [x] **Tag autocomplete.** `getKnownTags()` (admin client, no caching since the list is small) feeds a new `TagInput` client component. Suggests matches against the current trailing token; ArrowUp/Down navigates, Enter/Tab inserts.
- [x] **Inline draft→publish toggles.** Bulk bar covers single-row publish out of `/studio/drafts` without drilling into the editor. (Earlier `InlinePublish` button was superseded once selection-based publishing landed.)
- [x] **Chapter excerpt + subtitle parity.** Migration `0013_chapter_excerpt.sql` adds `excerpt text default ''` to chapters. Types, `mapChapter`, the chapter editor form, page query, and the `updateChapter` action all carry the new field through.
- [x] **RSS link in footer.** Plain `<a href="/feed.xml">` added to `SiteFooter` between Contact and Studio. `aria-label` explains it's the RSS feed.
- [x] **Image dedup.** `dedupUploadToCovers()` SHA-256s the bytes, stores under `covers/dedup/<hash>.<ext>`, and returns the existing public URL when the path already lists. Both `uploadCover` and `uploadStudioImage` use it. `removeWorkStorage` keeps sweeping legacy per-work prefixes but no longer touches the shared `dedup/` prefix (with a code comment explaining the trade-off).
- [x] **Save-success toast + last-saved tooltip.** SaveIndicator now flashes a green "saved" chip above the indicator on each `saving → saved` transition (auto-dismiss after 1.5s) and stamps a `title` tooltip with the full "Saved Sat, May 28, 3:42 PM" timestamp.
- [x] **Reader font-size control.** Already shipped in Phase 5 🟡 — kept here only as a historical note. The "Type" radiogroup in ThemeSwitcher writes sm/md/lg to `data-reading-size` on `<html>`; CSS keys off the attribute.
...
## Phase 6 — Migrations + env to apply (2026-05-29)

The Phase 5 🔵 sweep added several new SQL migrations and one new env var.
They live in `supabase/migrations/`. Run them in order via the Supabase SQL
Editor (or `supabase db push` once the GitHub Action's secrets are set).

- `0012_works_featured.sql` — `works.featured boolean` + partial index. Until applied, `getFeaturedWork()` falls back silently to "newest published" so the homepage stays up.
- `0013_chapter_excerpt.sql` — `chapters.excerpt text default ''`. Chapter editor + reader-side `Chapter.excerpt` already expect it.
- `0014_search.sql` — generated `search_tsv` columns + GIN indexes on works + chapters. The `/search` route depends on this; without it the route returns no results.
- `0015_work_versions.sql` — `work_versions` table. Studio versions page needs it.
- `0016_scheduled_publish.sql` — `works.scheduled_at` + partial index. The cron sweeps draft rows where `scheduled_at <= now()`.
- `0017_reader_state.sql` — `reader_state` table backing `/api/reader/state`.

### New env vars

- `CRON_SECRET` — random hex string, set on Vercel (Production + Preview). The scheduled-publish cron checks `Authorization: Bearer <CRON_SECRET>`; missing or mismatched headers return 401.
- GitHub repo secrets for the `supabase-db-push` Action: `SUPABASE_ACCESS_TOKEN`, `SUPABASE_PROJECT_REF`, `SUPABASE_DB_PASSWORD`.
